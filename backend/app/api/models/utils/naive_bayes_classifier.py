...
class NaiveBayesClassifierModel:
    def __init__(self, db, b2_filemanager, dataset_id, ignore_columns, target_column, model_id=None):
        ...
        # Store features for importance calculation
        self.X = X
    ...
    def train(self, model_name='naive_bayes_classifier_model.pkl', var_smoothing=1e-9):
    ...
    def evaluate(self):
        ...
        # Calculate feature importance using variance of each feature
        # Note: This is a simple approximation as Naive Bayes doesn't have built-in feature importance
        importance = np.var(self.X_train, axis=0)
        total_variance = np.sum(importance)
        importance = importance / total_variance  # Normalize to sum to 1
