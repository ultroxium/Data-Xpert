...
class LinearRegressionModel:
    def __init__(self, db, b2_filemanager, dataset_id, ignore_columns, target_column, model_id=None):
        ...
        # To Find importance (coefficients for linear regression)
        self.X = X
    ...
    def train(self, model_name='linear_regression_model.pkl'):
        target_type = type_of_target(self.y_train)
        if target_type not in ['continuous']:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, 
                detail="Target column should be continuous for linear regression."
            )
    ...
    def evaluate(self):
        ...
        importance = np.abs(self.model.coef_)  # Use absolute coefficients as importance
        ...
        result = {
            ...
            "model_equation": {
                ...
                "coefficients": dict(zip(self.X.columns, self.model.coef_.tolist()))
            }
        }
